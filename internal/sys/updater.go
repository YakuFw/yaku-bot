...
const (
	...
	RemoteVersionURL = "https://raw.githubusercontent.com/YakuFw/yaku-bot/main/version.txt"
)
...
cd /tmp
rm -rf yaku-bot
git clone https://github.com/YakuFw/yaku-bot.git
cd yaku-bot
export PATH=$PATH:/usr/local/go/bin
go mod tidy
go build -o /usr/local/bin/yaku-bot ./cmd/yaku-bot
systemctl restart yaku-bot
`
	err := os.WriteFile("/tmp/yaku-bot_update.sh", []byte(updateScript), 0755)
	if err != nil {
		return fmt.Errorf("error creando script de actualización: %v", err)
	}

	unitName := fmt.Sprintf("yaku-bot-updater-%d", time.Now().Unix())
	cmd := exec.Command("systemd-run", "--unit="+unitName, "/tmp/yaku-bot_update.sh")
	err = cmd.Start()
	if err != nil {
		// Fallback por si systemd-run falla
		cmdFallback := exec.Command("sh", "-c", `nohup /tmp/yaku-bot_update.sh > /dev/null 2>&1 &`)
