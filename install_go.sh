...
# =========================================================
# INSTALADOR UNIVERSAL V8.0.5: BOT TELEGRAM YAKU SSH 💎 (GO EDITION)
# =========================================================
...
PROJECT_DIR="/opt/yaku_bot"
ENV_FILE="$PROJECT_DIR/.env"
SERVICE_NAME="yaku-bot.service"
BIN_PATH="/usr/local/bin/yaku-bot"
...
install_bot() {
    ...
    if [ -z "${BOT_TOKEN:-}" ] || [ -z "${ADMIN_ID:-}" ]; then
        read -rp "🤖 Introduce el TOKEN de tu bot: " BOT_TOKEN
        read -rp "👤 Introduce el Chat ID del Super Admin: " ADMIN_ID
    fi
    ...
    # 2. Instalar Go 1.23 si no existe
    export PATH="$PATH:/usr/local/go/bin"
    if ! command -v go >/dev/null 2>&1; then
        log_info "Instalando GoLang 1.23..."
        ARCH="$(uname -m)"
        case "$ARCH" in
            x86_64) GOARCH="amd64" ;;
        ...
        wget -q "https://go.dev/dl/${GO_TAR}" || { log_error "No se pudo descargar Go."; exit 1; }
        rm -rf /usr/local/go
    ...
    cd /tmp
    rm -rf /tmp/yaku-bot
    git clone https://github.com/YakuFw/yaku-bot.git /tmp/yaku-bot || { log_error "Error al descargar el bot."; exit 1; }
    cd /tmp/yaku-bot

    log_info "Descargando módulos necesarios..."
    go mod tidy

    go build -o "$BIN_PATH" ./cmd/yaku-bot
    chmod +x "$BIN_PATH"
    rm -rf /tmp/yaku-bot
    cd ~
...
[Unit]
Description=Yaku Bot Telegram VPN Manager (Go Edition)
...
show_menu() {
    clear
    echo -e "${CYAN}=================================================="
    echo -e "       YAKU BOT INSTALLER (GO EDITION)"
    echo -e "==================================================${NC}"
